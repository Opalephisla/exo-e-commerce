...
if ($uploadOk == 0) {
    header("Location: ../index.php?categ=" . $_GET["categ"] . "&message=Le fichier n'a pas été téléchargé");
    // if everything is ok, try to upload file
} else if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
    $alt = basename(htmlspecialchars(basename($_FILES["fileToUpload"]["name"])), ".png");

    if (
        isset($_POST["nomprod"]) && !empty($_POST["nomprod"]) && isset($_POST["description"]) && !empty($_POST["description"]) && isset($_POST["prixht"]) && !empty($_POST["prixht"]) &&  isset($_POST["tva"]) && !empty($_POST["tva"]) && isset($_POST["prixttc"]) && !empty($_POST["prixttc"]) &&
